  ...
  return (
    <main className="gig-shell">
      ...
      <header className="gig-hero">
        <div><p>TomOS / Live music</p><h1>Gig Radar</h1></div>
      ...
      {sourceProblems.length > 0 && (
        <section className="gig-notice" aria-labelledby="gig-setup-title">
          <div><p>Setup required</p><h2 id="gig-setup-title">Connect the sources to start scanning.</h2></div>
      ...
      <section className="gig-stats" aria-label="Gig Radar status">
        ...
        <div><span>Sources online</span><strong>{Object.values(data.sourceHealth).filter((source) => source.status === "healthy").length}/2</strong></div>
      ...
      <section className="gig-section">
        ...
        {data.events.length ? <div className="gig-list">{data.events.map((event) => <EventCard event={event} key={event.id} />)}</div> : <div className="gig-empty"><span>♪</span><h3>No shows on the radar yet.</h3><p>Once Spotify and Ticketmaster are configured, followed artists and their Australian shows will appear here.</p></div>}
      </section>

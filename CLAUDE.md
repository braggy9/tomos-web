...
- Gig Radar is currently a request-time, read-only MVP. It has a Spotify OAuth connect/callback/disconnect flow and stores the refresh token durably, but still has no scheduled scan, notification delivery, or user event decisions, and persists nothing beyond the credential.
- Gig Radar remains **unconfigured in production**: as at 15 September 2026 `/gigs` reports `Sources available 0/2`. Spotify now has a Connect flow (`/api/spotify/connect`) that stores an encrypted refresh token in the dashboard-owned Neon project `tomos-dashboard`, so the remaining work is configuration only — `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `DATABASE_URL`, `TICKETMASTER_API_KEY` on Vercel Production, then Connect. Ticketmaster has still never been exercised; its state is unknown, not broken. See the Production Verification Record in `apps/dashboard/HANDOVER.md`.

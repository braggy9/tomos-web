...
- Gig Radar Phase 1 is merged and deployed but **blocked, not merely unverified**: as at 14 September 2026 the production `/gigs` page reports `Sources available 0/2` and `Spotify credentials are not configured`. The Spotify environment variables were never set on the Vercel project, so Ticketmaster has never been exercised and its state is unknown. The shared radar navigation is confirmed working. See the Production Verification Record in `apps/dashboard/HANDOVER.md`.
